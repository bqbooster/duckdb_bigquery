...
void BigQueryTableSet::AddColumn(ClientContext &context, BigQueryResult &result, BigQueryTableInfo &table_info,
	...
	auto column_type = BigQueryUtils::TypeToLogicalType(context, type_info);
...
ORDER BY table_name, ordinal_position;
	...
	vector<unique_ptr<BigQueryTableInfo>> tables;
	...
	while (result->Next()) {
		...
		if (!info || info->GetTableName() != table_name) {
			...
			info = make_uniq<BigQueryTableInfo>(schema, table_name);
		}
	...
	for (auto &tbl_info : tables) {
		auto table_entry = make_uniq<BigQueryTableEntry>(catalog, schema, *tbl_info);
...
ORDER BY table_name, ordinal_position;
	                                               ...
	                                               "${SCHEMA_NAME}", BigQueryUtils::WriteLiteral(schema_name)),
	                           "${TABLE_NAME}", BigQueryUtils::WriteLiteral(table_name));
}
...
unique_ptr<BigQueryTableInfo> BigQueryTableSet::GetTableInfo(ClientContext &context, BigQuerySchemaEntry &schema,
	...
	auto result = transaction.Query(query);
	auto table_info = make_uniq<BigQueryTableInfo>(schema, table_name);
	while (result->Next()) {
...
// should be unified
string BigQueryColumnsToSQL(const ColumnList &columns, const vector<unique_ptr<Constraint>> &constraints) {
	std::stringstream ss;
	...
	for (auto &column : columns.Logical()) {
		...
		ss << BigQueryUtils::WriteIdentifier(column.Name()) << " ";
		ss << BigQueryUtils::TypeToString(column.Type());
...
string GetBigQueryCreateTable(CreateTableInfo &info) {
	for (idx_t i = 0; i < info.columns.LogicalColumnCount(); i++) {
		auto &col = info.columns.GetColumnMutable(LogicalIndex(i));
		col.SetType(BigQueryUtils::ToBigQueryType(col.GetType()));
	}
	...
	if (!info.schema.empty()) {
		ss << BigQueryUtils::WriteIdentifier(info.schema);
		ss << ".";
	}
	ss << BigQueryUtils::WriteIdentifier(info.table);
	ss << BigQueryColumnsToSQL(info.columns, info.constraints);
	ss << ";";
	return ss.str();
}
...
optional_ptr<CatalogEntry> BigQueryTableSet::CreateTable(ClientContext &context, BoundCreateTableInfo &info) {
	...
	auto tbl_entry = make_uniq<BigQueryTableEntry>(catalog, schema, info.Base());
...
void BigQueryTableSet::AlterTable(ClientContext &context, RenameTableInfo &info) {
	...
	sql += " RENAME TO ";
	sql += BigQueryUtils::WriteIdentifier(info.new_table_name);
...
void BigQueryTableSet::AlterTable(ClientContext &context, RenameColumnInfo &info) {
	...
	sql += BigQueryUtils::WriteIdentifier(info.name);
	sql += " RENAME COLUMN  ";
	sql += BigQueryUtils::WriteIdentifier(info.old_name);
	sql += " TO ";
	sql += BigQueryUtils::WriteIdentifier(info.new_name);
...
void BigQueryTableSet::AlterTable(ClientContext &context, AddColumnInfo &info) {
	...
	sql += BigQueryUtils::WriteIdentifier(info.new_column.Name());
	sql += " ";
...
void BigQueryTableSet::AlterTable(ClientContext &context, RemoveColumnInfo &info) {
	...
	if (info.if_column_exists) {
		throw NotImplementedException("DROP COLUMN IF EXISTS not supported in BigQuery");
	}
	sql += BigQueryUtils::WriteIdentifier(info.removed_column);
	transaction.Query(sql);
}
...
void BigQueryTableSet::AlterTable(ClientContext &context, AlterTableInfo &alter) {
	...
	case AlterTableType::REMOVE_COLUMN:
		...
		break;
	default:
		throw BinderException("Unsupported ALTER TABLE type - BigQuery tables only "
